...
imgs = [img.permute(1,2,0) for img in images]
...
class VOCSegDataset(torch.utils.data.Dataset):
    def __init__(self, is_train, crop_size, voc_dir):
        ...
        self.features = [self.normalize_image(feature) for feature in self.fliter(features)]
        self.labels = self.fliter(labels)
    ...
    def __getitem__(self, index):
        feature, label = voc_rand_crop(self.features[index], self.colormap2label, *self.crop_size)

...
def read_csv_labels(fname):
    with open(fname, 'r') as f:
        lines = f.readlines()[1:]
    tokens = [l.restrip().split(',') for l in lines]  # restrip: 去掉换行符

    return dict((name, label) for name, label in tokens)

labels = read_csv_labels(os.path.join(data_dir, 'trainLabels.csv'))
print('# 训练样本：', len(labels))
print('# 类别： ', len(set(label.values())))
...
valis_ds, test_ds = [torchvision.datasets.ImageFolder(
    os.path.join(data_dir, 'train_valid_test', folder), 
...
def train(net, train_iter, valid_iter, num_epochs, lr, wd, devices, lr_period,
    ...
    measures = (f'train loss {metric[0]/metric[2]:.3f}, '
                f'train acc {metric[2]/metric[2]:.3f} ')
    if valid_iter is not None:
        measures += (f'valid acc {valis_acc:.3f} ')
...
for X, _ in test_iter:
    ...
    preds.append(torch.argmax(y_hat, dim=1).type(torch.int32).cpu().numpy())

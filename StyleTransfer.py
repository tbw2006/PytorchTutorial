...
d2l.set_figsize()
content_img = d2l.img.open('rainer.jpg')
d2l.plt.imshow(content_img)


#

style_img = d2l.img.open('autumn-oak.jpg')
...
class SynthesizedImage(nn.Module):
    def __init__(self, img_shape, **kwargs):
        ...
        self.weight = nn.Parameter(torch.randn(*img_shape))
...
def train(X, contents_Y, styles_Y, device, lr, num_epochs, lr_decay_epoch):
    ...
    for epoch in range(num_epochs):
        trainer.zero_grad()
        contents_Y_hat, styels_Y_hat = extract_features(
        ...
        if (epoch+1) % 10 == 0 :
            ...
            animator.add(epoch+1, [float(contents_l), 
                float(styles_l), float(tv_l)])
    return X
